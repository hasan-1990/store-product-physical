...
export default function NextRocketDashboard() {
  ...
  const loadStats = async () => {
    try {
      ...
      const [cacheRes, statusRes] = await Promise.all([
        fetch('/api/admin/next-rocket/cache'),
        fetch('/api/admin/next-rocket/optimize'),
      ]);
  ...
  const handleClearCache = async (type: 'all' | 'tag' | 'key', value?: string) => {
    // نمایش مدال تایید با تم خطرناک
    setModalConfig({
      title: '🗑️ پاک کردن کش',
      message: 'آیا مطمئن هستید که می‌خواهید کش را پاک کنید؟ این عملیات قابل بازگشت نیست!',
      type: 'danger',
      onConfirm: async () => {
        setShowModal(false);
        try {
          toast.loading('در حال پاک کردن کش...', { id: 'cache' });
          const url = `/api/admin/next-rocket/cache?type=${type}${value ? `&value=${value}` : ''}`;
          const res = await fetch(url, { method: 'DELETE' });
          const data = await res.json();

          if (data.success) {
            toast.success(data.message || '✨ کش با موفقیت پاک شد!', { id: 'cache' });
            await loadStats();
          } else {
            toast.error(data.error || '❌ خطا در پاک کردن کش', { id: 'cache' });
          }
        } catch (error) {
          console.error('خطا در پاک کردن کش:', error);
          toast.error('❌ خطا در انجام عملیات. لطفاً دوباره تلاش کنید.', { id: 'cache' });
        }
      }
    });
    setShowModal(true);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-purple-950 to-slate-950 p-8">
      ...
      {activeTab === 'dashboard' && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {/* Cache Stats - PREMIUM CARD */}
          <div className="group relative rounded-[2rem] overflow-hidden">
            {/* Animated Background Gradient */}
            <div className="absolute inset-0 bg-gradient-to-br from-amber-400 via-orange-500 to-red-500 opacity-0 group-hover:opacity-100 transition-opacity duration-700"></div>
            <div className="absolute inset-0 bg-gradient-to-br from-yellow-50 via-amber-50 to-orange-50 opacity-100 group-hover:opacity-0 transition-opacity duration-700"></div>
            
            {/* Glass Card */}
            <div className="relative bg-white/90 backdrop-blur-xl rounded-[2rem] shadow-2xl hover:shadow-amber-500/30 hover:shadow-3xl p-8 border-2 border-amber-100/50 transform hover:scale-105 hover:-translate-y-2 transition-all duration-500">
              <div className="flex items-center gap-5 mb-8">
                {/* Icon with Glow */}
                <div className="relative group-hover:scale-110 transition-transform duration-500">
                  <div className="absolute -inset-2 bg-gradient-to-br from-amber-400 via-yellow-500 to-orange-500 rounded-3xl blur-xl opacity-60 group-hover:opacity-80 animate-pulse"></div>
                  <div className="relative p-5 bg-gradient-to-br from-amber-400 via-yellow-500 to-orange-500 rounded-3xl shadow-xl shadow-amber-500/50">
                    <BoltIcon className="w-12 h-12 text-white drop-shadow-lg" />
                  </div>
                </div>
                <div>
                  <h3 className="text-3xl font-black bg-gradient-to-r from-amber-600 via-orange-600 to-red-600 bg-clip-text text-transparent">
                    کش سیستم
                  </h3>
              ...
              <div className="space-y-5 mb-8">
                {/* Stats Items with Premium Style */}
                <div className="group/item relative overflow-hidden rounded-2xl bg-gradient-to-r from-amber-50 to-orange-50 p-5 border-2 border-amber-200/50 hover:border-amber-400 transition-all duration-300 hover:shadow-lg">
                  <div className="absolute inset-0 bg-gradient-to-r from-amber-400 to-orange-400 opacity-0 group-hover/item:opacity-10 transition-opacity"></div>
                  <div className="relative flex justify-between items-center">
                    <span className="text-gray-700 font-bold text-lg">📦 تعداد آیتم‌ها:</span>
                ...
                <div className="group/item relative overflow-hidden rounded-2xl bg-gradient-to-r from-yellow-50 to-amber-50 p-5 border-2 border-yellow-200/50 hover:border-yellow-400 transition-all duration-300 hover:shadow-lg">
                  <div className="absolute inset-0 bg-gradient-to-r from-yellow-400 to-amber-400 opacity-0 group-hover/item:opacity-10 transition-opacity"></div>
                  <div className="relative flex justify-between items-center">
                    <span className="text-gray-700 font-bold text-lg">💾 سایز کل:</span>
                    <span className="text-2xl font-black text-yellow-700">
                      {stats.cache?.totalSize || '0 MB'}
                    </span>
                  </div>
                </div>
                
                <div className="group/item relative overflow-hidden rounded-2xl bg-gradient-to-r from-green-50 via-emerald-50 to-teal-50 p-5 border-2 border-green-300 hover:border-green-500 transition-all duration-300 hover:shadow-xl hover:shadow-green-500/20">
                  <div className="absolute inset-0 bg-gradient-to-r from-green-400 to-emerald-400 opacity-0 group-hover/item:opacity-10 transition-opacity"></div>
                  <div className="relative flex justify-between items-center">
                    <span className="text-gray-700 font-bold text-lg">🎯 Hit Rate:</span>
              ...
              {/* Clear Cache Button - PREMIUM */}
              <button
                onClick={() => handleClearCache('all')}
                className="group/btn relative w-full overflow-hidden bg-gradient-to-r from-red-500 via-rose-600 to-pink-600 text-white py-5 rounded-2xl transition-all duration-500 flex items-center justify-center gap-3 font-bold text-lg shadow-2xl hover:shadow-red-500/50 hover:scale-105 hover:-translate-y-1"
              >
                <div className="absolute inset-0 bg-gradient-to-r from-pink-400 to-red-400 opacity-0 group-hover/btn:opacity-100 transition-opacity duration-500"></div>
                <TrashIcon className="relative w-7 h-7 group-hover/btn:animate-bounce" />
                <span className="relative">پاک کردن کش</span>
              </button>
            </div>
          </div>
                  ...
                  <div className="relative flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-3xl font-black bg-gradient-to-r from-green-400 to-emerald-400 bg-clip-text text-transparent">
                        {stats.criticalCss?.savingsPercentage?.toFixed(1) || 0}%
                      </span>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex justify-center mb-6">
                <div className={`relative p-6 rounded-full ${
                  ...
                  {modalConfig.type === 'danger' ? (
                    <TrashIcon className="relative w-12 h-12 text-white drop-shadow-lg" />
          ...
          <label className="relative inline-flex items-center cursor-pointer" onClick={() => {
            ...
            <div className={`relative w-20 h-10 rounded-full transition-all duration-500 ${
              ...
              <div className={`absolute top-1 transition-all duration-500 ${
                ...
                settings.enabled ? 'rotate-[360deg] scale-110' : 'rotate-0'
              }`}>
                {settings.enabled ? (
