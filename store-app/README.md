...
## 1. Overview
**Store-App** is an enterprise-grade, full-stack eCommerce platform specifically engineered for digital products, scripts, CMS templates, downloadable assets, as well as physical goods. 

In addition to standard online store functionality, it features an **Automated Site Provisioning & Multi-Tenant Engine**, allowing customers to purchase website templates and deploy turnkey, isolated websites on their own custom domains with automated DNS verification and Nginx virtual host configurations.

---

## 2. Technical Architecture
Built upon a state-of-the-art dual-engine stack:
* **Frontend Web Application:** Powered by **Next.js 15.5** (App Router), **React 19**, **TypeScript 5**, and **Tailwind CSS 3.4**.
...
* **Storage & Caching:** **MongoDB 7.0** with high-throughput native driver connections, paired with **Redis 7** for lightning-fast memory caching and session persistence.
* **DevOps Ready:** Full Docker integration with multi-stage standalone builds, Nginx reverse proxy, automated Certbot SSL, and Uptime Kuma health monitoring.

---

## 3. Storefront & Customer Features
* **Catalog & Advanced Filtering (`/products`, `/categories`):** Multi-level faceted search by categories, pricing, discounts, ratings, and custom product attributes.
* **Modern Product Showcase (`/products/[slug]`):** Responsive image gallery, live interactive demo links, verified customer reviews with ratings, and an AI-driven "You May Also Like" recommendation engine.
* **Seamless Checkout (`/cart`, `/checkout`):** Real-time cart synchronization, percentage and fixed promo discount codes, dynamic shipping calculation, and streamlined digital checkout.
* **Payment Gateways (`/payment`):** Integrated with official Iranian payment services (**Zarinpal** & **Zibal**) with sandbox testing support.
* **Customer Portal (`/profile`, `/user`):**
  * Dedicated download repository with expiring secure download links.
  * My Sites manager (`/user/my-sites`): Automated DNS A-record and CNAME verification for provisioned custom domains.
  * Order history and printable/PDF digital invoices.
  * Support Ticket management with file attachments and priority tracking.
  * Address book and personal profile management.
  * Interactive Wishlist (`/wishlist`).

---

## 4. Admin Management Panel
Accessible via **`/admin`**, offering 50+ enterprise control modules:
* **Analytical Dashboard:** Real-time revenue metrics, order velocity, active users, and system health status.
* **Catalog Manager:** Physical and digital product creation, file uploads, automated WebP image transformation, and discount scheduling.
* **Order & Invoice Processing:** Multi-status order handling, payment verification logs, and automated invoice PDF generation.
* **Multi-Tenant Provisioning (`/admin/site-templates`, `/admin/site-instances`):** Packaging shop templates, monitoring active tenant instances, and managing domain configs.
* **Advanced SEO & Structured Data:** OpenGraph customization, 301/302 URL redirects, canonical tags, automated XML sitemaps, and rich Schema markup (Product, FAQ, Organization, Breadcrumb).
* **AI Chatbot (`/admin/chatbot`):** Powered by Google Gemini AI with customizable system prompts for 24/7 autonomous customer inquiries.
* **Visual Store Builder:** Live editors for homepage carousels, banner grids, trust badges, mega-menus, headers, and footers.
* **Background Task Scheduler (Cron):** Automated abandoned cart notifications, license cache invalidation, and automated database backups.
* **Communication Gateways:** SMS.ir fast-pattern verification service (OTP) and SMTP transactional email delivery.

---

## 5. License Engine & Security
* **Domain Lock Protection:** Proprietary PHP and Node.js licensing wrappers that bind downloaded scripts/templates to customer-authorized domains.
* **Signed Download Links:** Downloadable assets are protected behind temporary signed tokens, preventing link sharing or bandwidth theft.
* **Security Hardening:** Enterprise-level rate limiting, CSRF mitigation, sanitize filters against NoSQL injection, and strict Content Security Policies (CSP).

---

## 6. Multi-Tenant Provisioning
1. Customer purchases a site template and assigns their custom domain.
2. The system queries DNS records to verify proper pointing.
3. The Provisioning engine automatically:
   * Sets up isolated upload directories.
   * Creates an isolated MongoDB database and injects starter seed data.
   * Generates a tailored Nginx server block in `/etc/nginx/sites` and reloads Nginx gracefully.
   * Activates the tenant store with independent administrative credentials.
...
cp .env.example .env.local
# Edit your database credentials in .env.local

# 4. Run Next.js Frontend
npm run dev
# Frontend runs at http://localhost:3000

# 5. Run Express Core-Server (in another terminal)
npm run core:dev
# Core API runs at http://localhost:4000
```
...
| :--- | :--- | :--- |
| `MONGODB_URI` | MongoDB Connection String | `mongodb://localhost:27017/store-app` |
| `NEXTAUTH_SECRET` | NextAuth Session Encryption Key | 32+ character random string |
| `NEXTAUTH_URL` | Canonical URL of the application | `http://localhost:3000` |
| `JWT_SECRET` | Secret key for custom JWT tokens | Secure random token |
| `REDIS_ENABLED` | Toggle Redis caching layer | `true` / `false` |
| `REDIS_URL` | Redis server address | `redis://localhost:6379` |
| `SMSIR_API_KEY` | SMS.ir API Token | Your SMS.ir API Key |
| `ZARINPAL_MERCHANT_ID` | Zarinpal Payment Merchant ID | Gateway UUID |
| `CORE_API_URL` | Internal URL for Express Core-Server | `http://127.0.0.1:4000` |
